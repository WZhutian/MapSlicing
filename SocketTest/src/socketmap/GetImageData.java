...
public class GetImageData implements Runnable {
	 ...
	 */ public void setParameter(ThreadExportOption threadOpt ,client temp) {
		this.GetTemp=temp;
		...
		if (threadOpt.Index_bds == 1) {
			...
			int Cuted = threadOpt.First_bd + (threadOpt.Index_bds - 2) * threadOpt.Number_bds + threadOpt.cut_index;// 已经被切好的bundle数量
			int Rs = Cuted / threadOpt.bundleRows;// 计算已经切了多少行
			int Cs = Cuted - Rs * threadOpt.bundleRows;// 计算切到了第几列
			this.stRows = Rs;// this.edRows = threadOpt.edRows;
			this.stCols = Cs; // this.edCols=threadOpt.edCols;
	...
	@Override
	public void run() {
		System.out.println("The bEFORE"+GetTemp.RunningThread);
		GetTemp.cutdownThread();
		System.out.println("The lATER"+GetTemp.RunningThread+" STrow"+this.stRows+";stCols"+this.stCols);
			...
			for (int i = 0; i < 128; i++) {
				for (int j = 0; j < 128; j++) {
					...
					// 偏移量加4
					{
						...
						// sum=sum+(Etime-Stime);
						/*
						 * if(i==127&&j==127){ GetIntervalTime(sum);
						 * this.ExFunc.wirteLog("并行====线程号    " +
						 * this.index+"==bundlerow:" + stRows + "==bundlecol:" +
						 * stCols + "==finish==bundleIo:" + "==时间间隔==" + DD+"天"
						 * +HH+"小时"+MM+"分钟"+interval+"秒"); }
						 */
		...
		System.out.println(alltime + "总共的时间"); // 相减得出运行时间
		System.out.println(iotimeall + "io时间");
		System.out.println(requesttimeall + "请求时间");
		System.out.println(indextimeall + "索引bundlx时间");
		System.out.println(picturetimeall + "图片IO时间");
